...
//     if (!formData.photo) {
//         const message = currentServiceId
//             ? "Please upload an image again."
...
//     try {
//         const userId = localStorage.getItem("userid");
//         const endpoint = currentServiceId
//             ? `${ip}/moox_events/api/service/update-service`
//             : `${ip}/moox_events/api/service/add-service`;
...
//         formDataToSend.append("photo", formData.photo); // Add file data

//         if (currentServiceId) {
//             formDataToSend.append("id", currentServiceId);
//         }
...
//     setFormData({ name: service.title, description: service.status, photo: service.photo });
//     setCurrentServiceId(service.id);
//     setPreviewImage(service.photo);
...
const AddService = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.photo && !currentServiceId) {
    ...
    try {
      ...
      setServices(updatedResponse.data.events);
    } catch (error) {
      setError(
        "Failed to add/update service. " +
        (error.response?.data?.message || error.message)
  ...
  const handleEdit = (service) => {
    ...
    setCurrentServiceId(service.id);
    setPreviewImage(`${ip}/clients/${service.photo}`);
    setIsFormVisible(true);
  };
  ...
  const handleStatusChange = async (serviceId, currentStatus) => {
    try {
      const response = await axios.post(
        `${ip}/moox_events/api/service/change-service-status`,
        {
          event_id: serviceId,
          user_id: localStorage.getItem("userid"),
          status: !currentStatus,
  ...
  const handleDelete = async (serviceId) => {
    try {
      const userId = localStorage.getItem("userid");
      const response = await axios.post(
        `${ip}/moox_events/api/service/delete-service`,
        {
          event_id: serviceId,
          user_id: userId,
        }
              ...
              <div className="p-6 sm:p-8 relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-1 gap-2">
                    ...
                    <div className="relative">
                      <label className="block text-sm font-medium text-[#1a2a47] mb-2">
                        Service image <small>{currentServiceId && "(You need to select image again if you are editing)"}</small>
                      </label>
                      <div className="relative">
                        <input
                          type="file"
            ...
            {services.map((service) => (
              ...
              >
                ...
                <div className="absolute bottom-20 left-6">
                  ...
                  <p className="text-white line-clamp-2 text-md">
                  {service.status ? service.status.split(":")[0].trim() : "No Status Available"}
                  </p>
                </div>
                ...
                <div className="absolute bottom-6 right-6 flex items-center gap-2">
                  ...
                  <button
                    onClick={() => handleStatusChange(service.id, service.active)}
                    className={`flex items-center gap-2 max-w-[40px] hover:max-w-[130px] px-2 h-10 rounded-full transition-all duration-500 overflow-hidden group/status ${
                      service.active
                        ? "bg-[#1a2a47]/80 text-white hover:bg-[#d6af53]"
                        : "bg-[#d6af53]/80 text-white hover:bg-[#1a2a47]"
