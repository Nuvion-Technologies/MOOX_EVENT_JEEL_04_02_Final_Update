...
const BlogManagement = () => {
  ...
  const handleAddBlog = async (e) => {
    ...
    try {
      ...
      formDataToSend.append("description", newBlog.description);
      formDataToSend.append("readTime", newBlog.read_time);
      formDataToSend.append("date", newBlog.date);
      formDataToSend.append("author", newBlog.author);
      formDataToSend.append("tags", newBlog.tags);
      formDataToSend.append("category", newBlog.categoryName);
      // formDataToSend.append("profile_photo", newBlog.profile_photo);
      // formDataToSend.append("photo1", newBlog.photo1);
      // formDataToSend.append("photo2", newBlog.photo2);
      // formDataToSend.append("photo3", newBlog.photo3);
      // formDataToSend.append("photo4", newBlog.photo4);
      // formDataToSend.append("photo5", newBlog.photo5);

      // Add photos only if they exist
      if (newBlog.profile_photo) {
        formDataToSend.append("profile_photo", newBlog.profile_photo);
      }
      if (newBlog.photo1) {
        formDataToSend.append("photo1", newBlog.photo1);
  ...
  const handleToggleStatus = async (id, status) => {
    try {
      ...
      // Updated request body structure to match API expectations
      const requestBody = {
        user_id: user_id,
        event_id: id,  // Changed from blog_id to _id
        status: !status  // Using status instead of active
      };

      // Make the API call with the correct endpoint
      const response = await axios.post(
        `${ip}/moox_events/api/blogs/change-blog-status`,  // Changed back to original endpoint
        requestBody
      );
  ...
  const handleImageUpload = async (e, field) => {
    ...
    if (file) {
      try {
        const processedFile =
          file.size > 51200 ? await compressImage(file) : file;

        if (processedFile) {
          // Store the actual File object in newBlog state
          setNewBlog((prev) => ({
            ...prev,
            [field]: processedFile,
          }));
              ...
              >
                ...
                <div className="absolute bottom-24 left-6 right-6">
                  <div className="flex items-center gap-4 mb-4">
                    <img
                      src={`${ip}/blogs/${blog.profile_photo}`}
                      alt={blog.author}
